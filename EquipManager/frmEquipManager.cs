...
namespace EquipManager
{
    ...
    public partial class frmEquipManager : Form
    {
        ...
        void ServerProcess()
        {
            while(true)
            {
                if(listener.Pending()) // 접속 요청이 있는가? : non-blocking
                {
                    Socket sockT = listener.AcceptSocket(); // 외부로부터의 접속 요청 수락 : Blocking Method - 아무것도 못하고 기다리는 중
                    socks.Add(sockT);
                }
                Thread.Sleep(100);
            }
        ...
        void ReadProcess()
        {
            while(true)
            {
                ...
                {
                    Socket lsock = socks[i]; // 각각의 소켓들에 대한 상태 확인(연결 상태)
        ...
        string conString = @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=E:\SW기술진흥협회 교육\C++ 프로젝트 파일\EquipManager\EquipDataBase.mdf;Integrated Security=True;Connect Timeout=30";
        ...
        void ReadProc(Socket ss, byte[] bArr)
        {
            ...
            string sBatt    = str.Substring(17, 5); // float  : 5  16
            ...
            // - 기존 데이터가 없다면 ---> insert
            // - 기존 데이터가 있다면 ---> update
            // 기존 기록 중 가장 마지막 기록을 검색하여 update
            // update fStatus set fBatt=00.00 select top 1 * from fStatus where fCode=00001 order by fTime desc

            string sql;
            int n = (int)db1.Get($"select count(*) from fStatus where fCode='{sCode}'"); // fStatus에 있는 sCode와 fCode의 개수 조회
            
            // int n = (int)sqlCmd.ExecuteScalar(); // insert인지 update인지 명령 구분

            if(n == 0) // 기존 fCode가 없음 --> insert
            {
                sql = $"insert into {sTable} Values ({sCode},'{sModel}','{sLine}'," +
                      $"'{sBatt}','{sState}','{sCount}'," +
                      $"'{sTemp}','{sHum}','{sWind}'," +
                      $"'{sOzon}','{sAir}','{sTotal}','{sTime}')";
            }
            else // 기존 fCode가 있음
            {
                string st1 = db1.Get($"select top 1 * from {sTable} where fCode='{sCode}' order by fTime desc").ToString();
                string st2 = DateTime.Parse(st1).ToString("yyyy-MM-dd HH:mm:ss"); //'2021-04-26 9:18:46'

                sql = $"update {sTable} set fModel='{sModel}',fLine='{sLine}',fBatt='{sBatt}'," +
                      $"fStatus='{sState}',fCount='{sCount}',fTemp='{sTemp}'," +
                      $"fHum='{sHum}',fWind='{sWind}',fOzon='{sOzon}'," +
                      $"fAir='{sAir}',fTotal='{sTotal}',fTime='{sTime}' " +
        ...
        iniFile inif = new iniFile(".\\EquipManager.ini");
        private void frmEquipManager_Load(object sender, EventArgs e)
        {
            int x1, x2, y1, y2;
            tbServerPort.Text = inif.GetString("Server", "Port", "9001");
            ...
            tabControl1.SelectedIndex = int.Parse(inif.GetString("Form", "TabIndex", "1"));

            conString = inif.GetString("Database", "ConnectionString", @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=E:\SW기술진흥협회 교육\C++ 프로젝트 파일\EquipManager\EquipDataBase.mdf;Integrated Security=True;Connect Timeout=30");
        ...
        {
            inif.SetString("Server", "Port", tbServerPort.Text);
            ...
            inif.SetString("Form", "TabIndex", $"{tabControl1.SelectedIndex}");
            inif.SetString("Database", "ConnectionString", conString);
        ...
        void SetGrid()
        {
            GridStatus.DataSource = (DataTable)db2.RunSql("select * from fStatus");
        }
        ...
        private void GridStatus_RowHeaderMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            DataTable dt = (DataTable)db2.RunSql($"select * from EquipInfo where fCode='{sCode}'");
